@php
    ...
    $sistemaPages = ['home', 'lojas', 'sobre', 'ofertas', 'trabalhe_conosco', 'global'];
    $dynamicPages = PageContent::distinct()->whereNotIn('page', $sistemaPages)->orderBy('page')->pluck('page');
@endphp
    ...
    <div class="container py-5">
        <div class="row text-white" style="color: var(--gold);">
            ...
            <div class="col-md-3 coluna-footer">
                ...
                <a href="{{ env('RH_SITE_URL', 'http://localhost:8001') }}" target="_blank">Trabalhe conosco</a>
                <a href="{{ $cadastroUrl }}" target="_blank">Cadastre-se</a>
                <a href="{{ route('site.lojas') }}">Nossas Lojas</a>
                @foreach($dynamicPages as $dp)
                @php
                    $titulo = PageContent::where('page', $dp)->where('section', 'titulo')->value('content');
                @endphp
                <a href="{{ route('site.pagina', $dp) }}">{{ $titulo ?? $dp }}</a>
                @endforeach
            </div>
            <div class="col-md-3 coluna-footer">
